...
OUTPUT_JSON = "persona_emma_cleaned.json"
OUTPUT_MD = "persona_emma_cleaned.md"

def curate_facts(entry_facts, core_facts, exit_facts, top_entry=7, top_core=10, top_exit=5):
    """Trim and simplify facts for Emma persona."""
    def simplify(fact):
        # Keep only the part before the first '|', which usually has Wikipedia summary
...
def main():
    ...
    emma = next((p for p in narratives if p["Persona"] == "Emma"), None)
    if not emma:
        print("[ERROR] No Emma narrative found!")
        return

    # Curate and simplify Emma's facts
    entry_facts, core_facts, exit_facts = curate_facts(
        emma["EntryFactsUsed"], emma["CoreFactsUsed"], emma["ExitFactsUsed"]
    )

    prompt = f"""
You are a friendly museum storyteller.

Persona: Emma (Curious Novice)
Goal: Accessible, emotional, empathy-driven learning.
Tone: Light, story-like, easy for a first-time visitor.
...
""".strip()
    ...
    print("[INFO] Generating cleaned Emma narrative...")
    ...
    with open(OUTPUT_MD, "w", encoding="utf-8") as f:
        f.write("# Persona Narrative: Emma\n\n" + cleaned)

    print(f"[INFO] Cleaned Emma narrative saved:\n- {OUTPUT_JSON}\n- {OUTPUT_MD}")
