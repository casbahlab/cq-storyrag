...
# ----------------------- Topic modeling (LDA) -----------------------
# Optional, lightweight topic similarity using scikit-learn's LDA.
# We fit a small LDA over the union of sentences + context for each beat,
# then compute similarity as (1 - Jensen-Shannon divergence) between the
# topic distributions of a pair (query, candidate).
def _safe_import_sklearn_for_topics():
    try:
...
def _js_divergence(p, q, eps=1e-12):
    ...
    m = 0.5 * (p + q)
    def _kl(a, b):
        return (a * _np.log(a / b)).sum()
    return 0.5 * _kl(p, m) + 0.5 * _kl(q, m)

def _topic_similarity(vec_a, vec_b):
    # Convert JS divergence to a bounded similarity in [0,1].
    # JS divergence is in [0, ln(2)] for discrete distributions.
    import numpy as _np
    jsd = float(_js_divergence(vec_a, vec_b))
    # Normalize by ln(2) to map to [0,1], then invert.
    return max(0.0, 1.0 - (jsd / _np.log(2.0)))

def fit_topic_vectors(sent_list, ctx_list, n_topics=8, max_features=5000):
    LDA, CV, _np, err = _safe_import_sklearn_for_topics()
    if err is not None:
        return {}, {}, err
    docs = list(map(strip_noise, list(sent_list or []) + list(ctx_list or [])))
    docs = [d for d in docs if d]
    if not docs:
        return {}, {}, None
    n_topics = max(2, int(min(n_topics, len(docs)//2 or 2)))
    cv = CV(max_features=max_features, stop_words='english', ngram_range=(1,2))
    X = cv.fit_transform(docs)
    lda = LDA(n_components=n_topics, learning_method='batch', random_state=0)
    W = lda.fit_transform(X)  # shape: [n_docs, n_topics], rows sum to 1
    sent2vec, ctx2vec = {}, {}
    # map back to original strings (we use exact string keys)
    n_s = len(sent_list or [])
    for i, s in enumerate(sent_list or []):
        sent2vec[s] = W[i]
    for j, c in enumerate(ctx_list or []):
        ctx2vec[c] = W[n_s + j]
    return sent2vec, ctx2vec, None
...
def compute_feats(a: str, b: str, topic_a=None, topic_b=None) -> Dict[str, float]:
    tf = tfidf_cosine(a, b)
    ...
    topic = 0.0
    if topic_a is not None and topic_b is not None:
        try:
            topic = float(_topic_similarity(topic_a, topic_b))
        except Exception:
            topic = 0.0
    return {
        "tfidf": tf,
...
def _fuse_components(components: List[float], method: str) -> float:
    if not components: return 0.0
    if method == "max":  return max(components)
    if method == "mean": return sum(components) / len(components)
    if method == "sum":  return sum(components)              # CombSUM-like
    if method == "mnz":
        nz = sum(1 for x in components if x > 0)             # CombMNZ-like
        return (sum(components) * nz) if nz else 0.0
...
def best_match(query: str, candidates: List[str],
               ...
               topic_q_vecs: Dict[str, Any] | None = None,
               topic_c_vecs: Dict[str, Any] | None = None,
               fusion: str = "max",
               rrf_k: int = 60) -> Tuple[int, str, Dict[str,float]]:
    feats_list = []
    for i, c in enumerate(candidates):
        ta = topic_q_vecs.get(query) if (use_topic and topic_q_vecs) else None
        tb = topic_c_vecs.get(c)     if (use_topic and topic_c_vecs) else None
        feats = compute_feats(query, c, ta, tb)
        feats_list.append((i, c, feats))

    # RRF ranks per metric (if selected)
    rrf_scores = None
    if fusion == "rrf":
        rrf_scores = {i: 0.0 for i, _, _ in feats_list}
        def ranks_for(metric: str):
            idx_sorted = sorted(feats_list, key=lambda t: t[2][metric], reverse=True)
            return {i: rank for rank, (i, _, _) in enumerate(idx_sorted, start=1)}
        metrics = []
        if use_tfidf: metrics.append("tfidf")
        if use_char3: metrics.append("char3")
        if use_topic:  metrics.append("topic")
        for m in metrics:
            ranks = ranks_for(m)
            for i in rrf_scores:
                rrf_scores[i] += 1.0 / (rrf_k + ranks[i])

    best_i, best_c, best_sig = -1, "", -1.0
    best_feats = {"tfidf":0,"char3":0,"topic":0,"names":0,"nums":0,"years":0}
    for i, c, feats in feats_list:
        comps = []
        if use_tfidf: comps.append(feats["tfidf"])
        if use_char3: comps.append(feats["char3"])
        if use_topic:  comps.append(feats["topic"])
        base = rrf_scores[i] if (fusion == "rrf" and rrf_scores is not None) else _fuse_components(comps, fusion)
        sig = base + 0.06*feats["names"] + 0.04*max(feats["nums"], feats["years"])
        if sig > best_sig:
            best_sig = sig; best_i = i; best_c = c; best_feats = feats
    return best_i, best_c, best_feats


def decision_from_feats(feats: Dict[str,float],
                        tf_th: float, cj_th: float,
                        topic_th: float | None = None,
                        use_tfidf: bool = True,
                        use_char3: bool = True,
                        use_topic: bool = False,
                        decision: str = "any",
                        vote_k: int = 2) -> Tuple[bool, float, str, str, bool, bool, bool]:
    anchor_ok = (feats["names"] >= 1.0) and (feats["nums"] >= 1.0 or feats["years"] >= 1.0)
    tf_ok = (feats["tfidf"] >= tf_th) if use_tfidf else False
    cj_ok = (feats["char3"] >= cj_th) if use_char3 else False
    tp_ok = (feats["topic"] >= topic_th) if (use_topic and topic_th is not None) else False
    checks = [x for x, use in [(tf_ok,use_tfidf),(cj_ok,use_char3),(tp_ok,use_topic)] if use]

    if decision == "all":
        lexical_ok = all(checks) if checks else False
        rule = "all" if lexical_ok else "none"
    elif decision == "vote":
        k = max(1, min(int(vote_k), len(checks) if checks else 1))
        votes = sum(1 for x in checks if x)
        lexical_ok = (votes >= k)
        rule = f"vote({votes}/{k})"
    else:
        lexical_ok = any(checks) if checks else False
        rule = "any" if lexical_ok else "none"

    lens = []
    if use_tfidf: lens.append(("tfidf", feats["tfidf"]))
    if use_char3: lens.append(("char3", feats["char3"]))
    if use_topic:  lens.append(("topic", feats["topic"]))
    max_metric, L = (max(lens, key=lambda kv: kv[1]) if lens else ("none", 0.0))

    passed = bool(anchor_ok or lexical_ok)
    if anchor_ok and not lexical_ok: rule = "anchor"
    elif anchor_ok and lexical_ok:   rule = f"anchor+{rule}"
    return passed, float(L), rule, max_metric, tf_ok, cj_ok, tp_ok
...
def main():
    ...
    # metric toggles
    ap.add_argument("--use-tfidf", type=int, choices=[0,1], default=1, help="Enable TF-IDF cosine (1) or disable (0)")
    ap.add_argument("--use-char3", type=int, choices=[0,1], default=1, help="Enable char-3 Jaccard (1) or disable (0)")
    ap.add_argument("--use-topic", type=int, choices=[0,1], default=0, help="Enable topic modeling similarity (1) or disable (0)")
    ap.add_argument("--topic-th", type=float, default=0.30, help="Support topic similarity threshold (if enabled)")
    ap.add_argument("--topic-k", type=int, default=8, help="Number of LDA topics (per beat)")
    ap.add_argument("--topic-max-features", type=int, default=5000, help="Max vocabulary size for topic model")
    ...
    ap.add_argument("--cov-topic-th", type=float, default=None, help="Coverage topic similarity threshold (defaults to --topic-th)")
    ...
    # fusion strategy
    ap.add_argument("--fusion", choices=["max", "mean", "sum", "mnz", "rrf"], default="max",
                    help="How to combine enabled signals when ranking candidates")
    ap.add_argument("--rrf-k", type=int, default=60, help="RRF constant k (larger -> flatter)")

    # decision strategy
    ap.add_argument("--decision", choices=["any", "vote", "all"], default="any",
                    help="Pass rule over enabled metrics (in addition to anchors)")
    ap.add_argument("--vote-k", type=int, default=2,
                    help="If --decision vote, require at least K metrics over threshold")
    ...
    cov_topic = args.cov_topic_th if args.cov_topic_th is not None else args.topic_th
    ...
    for rec in recs:
        ...
        # Topic vectors (optional)
        topic_sent_vecs = {}
        topic_ctx_vecs = {}
        topic_err = None
        if int(args.use_topic) == 1:
            topic_sent_vecs, topic_ctx_vecs, topic_err = fit_topic_vectors(sents, ctx_clean, n_topics=args.topic_k, max_features=args.topic_max_features)
            if topic_err is not None:
                # If sklearn not installed, gracefully disable topic use.
                print(f"[WARN] Topic modeling disabled due to: {topic_err}")
                args.use_topic = 0
        ...
        for si, s in enumerate(sents):
            ...
            best_i, best_ctx, feats = best_match(
                s, candidate_ctx,
                use_tfidf=bool(args.use_tfidf), use_char3=bool(args.use_char3), use_topic=bool(args.use_topic),
                topic_q_vecs=topic_sent_vecs, topic_c_vecs=topic_ctx_vecs)
            supported, L, rule, max_metric, tf_ok, cj_ok, tp_ok = decision_from_feats(
                feats, args.tf_th, args.cj_th, topic_th=(args.topic_th if args.use_topic else None),
                use_tfidf=bool(args.use_tfidf), use_char3=bool(args.use_char3), use_topic=bool(args.use_topic),
                decision=args.decision, vote_k=int(args.vote_k))
            # (Do the same for the coverage path)

            support_rows.append({
                "beat_index": bidx, "beat_title": bttl, "sentence_idx": si,
                "sentence": s, "best_evidence": best_ctx,
                "tfidf": feats["tfidf"], "char3": feats["char3"], "topic": feats.get("topic", 0.0),
                "name_overlap": feats["names"], "num_overlap": feats["nums"], "year_overlap": feats["years"],
                "L": L,
                "canon_enabled": (not args.no_canon),
        "use_tfidf": bool(args.use_tfidf), "use_char3": bool(args.use_char3), "use_topic": bool(args.use_topic),
                "supported": bool(supported),
                "tfidf_ok": int(tf_ok), "char3_ok": int(cj_ok), "topic_ok": int(tp_ok),
                "rule": rule, "max_metric": max_metric,
            })
        ...
        if args.emit_coverage:
            for ci, c in enumerate(ctx_clean):
                ...
                best_i, best_sent, feats = best_match(
                    c, candidate_sents,
                    use_tfidf=bool(args.use_tfidf), use_char3=bool(args.use_char3), use_topic=bool(args.use_topic),
                    topic_q_vecs=topic_ctx_vecs, topic_c_vecs=topic_sent_vecs)
                covered, L, rule, max_metric, tf_ok, cj_ok, tp_ok = decision_from_feats(
                    feats, args.tf_th, args.cj_th, topic_th=(args.topic_th if args.use_topic else None),
                    use_tfidf=bool(args.use_tfidf), use_char3=bool(args.use_char3), use_topic=bool(args.use_topic),
                    decision=args.decision, vote_k=int(args.vote_k))
                coverage_rows.append({
                    "beat_index": bidx, "beat_title": bttl, "evidence_idx": ci,
                    "evidence": c, "best_sentence": best_sent, "topic": feats.get("topic", 0.0),
                    "tfidf": feats["tfidf"], "char3": feats["char3"], "topic": feats.get("topic", 0.0),
                    "name_overlap": feats["names"], "num_overlap": feats["nums"], "year_overlap": feats["years"],
    ...
    if getattr(args, "emit_near", False) and not df_sup.empty:
        ...
        near_cols = ["beat_index", "beat_title", "sentence_idx", "L", "tfidf", "char3", "topic",
